...
export default function TeacherChatPage() {
  ...
  const refresh = useCallback(async () => {
    if (!userId) return;
    try {
      setConversations(await listConversations(userId, "teacher"));
    } catch (err) {
  ...
  return (
    <div style={{ display: "flex", gap: 16, height: "calc(100vh - 140px)", minHeight: 400 }}>
      ...
      <div style={{ width: 280, minWidth: 280, display: "flex", flexDirection: "column", gap: 8 }}>
        <h3 style={{ margin: "0 0 4px" }}>Student Conversations</h3>
        <div style={{ flex: 1, overflow: "auto", border: "1px solid var(--border)", borderRadius: 10, padding: 8 }}>
          {conversations.length === 0 ? (
            <p className="muted" style={{ fontSize: 13, textAlign: "center", marginTop: 16 }}>
              No conversations yet. Students will appear here when they message you.
            </p>
          ) : (
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0 }}>
        ...
        {!activeId ? (
          <div className="card" style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center" }}>
            <p className="muted" style={{ fontSize: 15 }}>Select a conversation to start chatting.</p>
          </div>
        ) : (
          <>
